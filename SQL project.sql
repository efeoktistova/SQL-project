...
-- добавление должностей к таблице сотрудников
...
-- проверка наименований должностей сотрудников
select *
...
-- группы меню, связь с таблицей menu_books
...
-- требование уникальности к названию блюд, обязательное указание цены, связь с таблицей menu_groups
...
-- ЗАКАЗЫ
--столики и кол-во посадочных мест
...
-- заказы, связи с таблицами столики и сотрудники (официанты)
...
-- ограничение, чтобы в качестве официанта не указывались бухгалтер и уборщик
alter table restaraunt.orders
add constraint waiter_id check (waiter_id != 10 and waiter_id != 9);

-- внесение данных за прошлый период
...
-- добавление столбца Статус заказа, ограничения: выбор только из 2х значений, по умолчанию Active
alter table restaraunt.orders
...
-- в целях внесения данных за прошлый период
update restaraunt.orders 
set status = 'Closed' where order_active <= current_date;

-- проверка работы ограничения (введение статуса с ошибкой)
update restaraunt.orders 
...
-- элементы заказов, связь «многие ко многим»
...
-- СКИДКИ
-- программа лояльности, типы карт
...
-- добавление клиентов в таблицу orders
alter table orders 
...
-- расчёт суммы заказа без скидки (amount) и с учётом скидки (invoice)
...
-- к одному счёту может относиться несколько платежей
...
-- закрытие заказа, добавление скидкочной карты
update restaraunt.orders 
...
-- представление «Список открытых заказов»
...
-- представление «Список сотрудников с указанием должностей»
...
-- представление «Рейтинг сотрудников»
--explain
...
-- представление «Показ нарушений» (сумма платежа не равна тек.сумме счёта)
...
-- ИНДЕКСЫ 
--оптимизация представления "Выручка за сегодня"
create index dpay_indx on payments(date_pay);

--оптимизация представления "Список сотрудников с указанием должностей" и "Рейтинг сотрудников"
create index lnameempl_indx on employee(lastname);
create index fnameempl_indx on employee(firstname);


--ТРИГГЕРЫ
-- триггер последнее изменение таблицы Orders
...
-- триггер журнал логов добавление и обновление таблиц orders и payments (запись одной строкой)
create table audit (
...
-- триггер формирования Журнала нарушений
